...
def create_pre_spawn_hook(base_path, uid=JOVYAN_UID, mode=0o755):
    def pre_spawn_hook(spawner):
        ...
        for directory in [CONFIG_FOLDER, WORK_FOLDER]:
            # create user directory if it does not exist
            volume_path = os.path.join(base_path, username, directory)
...
def tljh_custom_jupyterhub_config(c):
    ...
    c.DockerSpawner.volumes = {
        os.path.join(VOLUMES_PATH, "{username}", CONFIG_FOLDER): os.path.join(
            "/home/jovyan", CONFIG_FOLDER
        ),
